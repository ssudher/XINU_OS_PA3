...
struct	pentry	{
	...
	//locks
	int pinh;
	int locksheld[NLOCKS];		// has the info of which all locks
					// are being held by this process
					// -1 -> not held, 0 -> READ lock
					// 1 -> WRITE lock
	unsigned long waittime;
	int lock_prio;
	int waiting_lock;
	unsigned long reference_time;   //this is to store what time the process was created
					//so it can be compared to the time lock was created
};
